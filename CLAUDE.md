...
## Minimal Admin Panel
The admin panel provides lightweight submission management:
- **Dashboard** — Overview stats and recent submissions from `contact_submissions` table
- **Submissions** — Full CRUD for contact form submissions with status tracking (new → contacted → qualified → converted)
- **Calendar** — Simple calendar view (currently uses empty data, ready for future events)

## Database
Only the `contact_submissions` table is actively used. Admin auth uses Supabase Auth with middleware protection.

## Technical Notes
- Service role key (`SUPABASE_SERVICE_ROLE_KEY`) bypasses RLS for admin server-side operations
- Dev command uses `next dev` (not `--turbopack`) due to Google Fonts compatibility
- Glass morphism dropdowns need explicit `bg-gray-900` for readability
- All pricing data sourced from `data/packages.ts`, all FAQ data from `data/faq.ts`